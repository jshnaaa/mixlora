#!/usr/bin/env python3
"""
Test script to verify parameter freezing logic for MixLoRA training.
"""

import sys
import os

def test_mixlora_only_parameter_logic():
    """Test the train_mixlora_only parameter logic."""
    print("🔍 Testing train_mixlora_only parameter logic...")
    ...
    try:
        ...
        # Check if train_mixlora_only logic is implemented
        if 'if self.args.train_mixlora_only:' in content:
            print("✓ train_mixlora_only parameter logic found")
        else:
            print("✗ train_mixlora_only parameter logic missing")
            return False

        # Check if it correctly identifies MoE components
        if "'moe_gate', 'experts'" in content and "'lora_A', 'lora_B'" in content:
            print("✓ Correctly identifies MoE components (moe_gate, experts, lora_A, lora_B)")
        else:
...
def test_shell_script_integration():
    """Test shell script integration with train_mixlora_only."""
    ...
    try:
        ...
        # Check if --train_mixlora_only is passed when LoRA weights exist
        if '--train_mixlora_only' in content:
            print("✓ Shell script passes --train_mixlora_only parameter")
        else:
            print("✗ Shell script missing --train_mixlora_only parameter")
            return False

        # Check the updated message
        if 'freeze pretrained LoRA and only train MixLoRA MoE components' in content:
...
def simulate_parameter_classification():
    """Simulate parameter classification for MixLoRA training."""
    print("\n🔍 Simulating parameter classification...")

    try:
        # Simulate typical parameter names in a MixLoRA model
        parameter_names = [
            ...
            # MixLoRA MoE components (should be trainable)
            "model.layers.0.mlp.moe_gate.weight",  # Router
            "model.layers.0.mlp.experts.0.gate_proj.lora_A.weight",  # Expert 0 LoRA
            ...
            "model.layers.0.mlp.experts.1.gate_proj.lora_A.weight",  # Expert 1 LoRA
        ...
        def should_be_trainable(param_name):
            """Simulate the train_mixlora_only logic."""
            # Only train MixLoRA MoE components: experts LoRA + router
            moe_components = ['moe_gate', 'experts']
            lora_parts = ['lora_A', 'lora_B', 'moe_gate']

            has_moe_component = any(component in param_name for component in moe_components)
            has_lora_part = any(lora_part in param_name for lora_part in lora_parts)

            return has_moe_component and has_lora_part
        ...
        all_moe_related = all('moe_gate' in name or 'experts' in name for name in actual_trainable)
        if all_moe_related:
...
def test_training_efficiency():
    ...
    try:
        ...
        mixlora_moe_params = 50_000_000     # ~50M MoE parameters (estimated)

        # Without freezing: train everything
        without_freezing = total_model_params + base_lora_params + mixlora_moe_params

        # With freezing: only train MoE components
        with_freezing = mixlora_moe_params
        ...
        print(f"  - MixLoRA MoE parameters: {mixlora_moe_params:,}")
...
def main():
    """Run all parameter freezing tests."""
    print("🚀 Testing MixLoRA parameter freezing configuration...\n")

    tests = [
        ("MixLoRA Only Parameter Logic", test_mixlora_only_parameter_logic),
        ("Shell Script Integration", test_shell_script_integration),
        ("Parameter Classification Simulation", simulate_parameter_classification),
    ...
    print("\n" + "="*60)
    print("📋 PARAMETER FREEZING TEST SUMMARY")
    print("="*60)
    ...
    if passed == len(tests):
        print("\n🎉 Parameter freezing configuration is correct!")
        print("\n📋 Training configuration:")
        print("  ✅ Pretrained LoRA weights: FROZEN")
        print("  ✅ Base model parameters: FROZEN")
        print("  ✅ MixLoRA router (moe_gate): TRAINABLE")
        print("  ✅ MixLoRA experts LoRA: TRAINABLE")
        print("  ❄️  All other parameters: FROZEN")
        print("\n🚀 run_custom_training will now only train MoE components!")
        print("\n💡 Usage:")
        print("  ./run_custom_training.sh llama 2 2 mixlora")
